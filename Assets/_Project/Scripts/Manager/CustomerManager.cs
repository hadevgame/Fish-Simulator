...
public class CustomerManager : Singleton<CustomerManager>
{
    ...
    private IEnumerator SpawnCustomersRoutine()
    {
        while (true)
        {
            if (LevelManager.Ins.CurrentLevel <= 3) spawnTime = 4f;
            else if (LevelManager.Ins.CurrentLevel <= 7) spawnTime = 3.5f;
            else spawnTime = 2.5f;
            ...
            if (StoreManager.Instance.GetState())
            {
                if (customer.IsNearStore() && customer.currentState is RoamingState)
                {
                    ...
                    if (tank != null)
                    {
                        ...
                        AudioManager.Ins.PlaySFX(StoreManager.Instance.AudioSO.GetAudioClip("ES"));
                        yield break;
                    }
                }
            }
